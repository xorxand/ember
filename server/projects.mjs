...
export async function rootPath(value) {
...
export async function safePath(root, relative, { write = false } = {}) {
  if (typeof relative !== "string" || relative.includes("\0"))
    throw new Error("Use a path within the project.");
  const canonical = await fs.realpath(root);
  if (path.isAbsolute(relative)) relative = path.relative(canonical, relative);
  const parts = relative.split(/[\\/]/);
  if (parts.some((p) => p === ".." || p === ".git" || sensitive(p)))
    throw new Error("This path is outside the allowed project files.");
  const candidate = path.resolve(canonical, relative);
  const inside = (value) =>
    value === canonical || value.startsWith(canonical + path.sep);
  ...
  for (const part of path
    ...
    try {
      ...
      if (!inside(actual))
        throw new Error("Symlinks cannot escape the project.");
    } catch (e) {
...
export async function listFiles(root, relative = "") {
  ...
  return entries
    .filter(
      (e) => !ignored.has(e.name) && !sensitive(e.name) && !e.isSymbolicLink(),
    )
    .map((e) => ({
      name: e.name,
...
export async function readFile(root, relative) {
  ...
  if (!stat.isFile() || stat.size > 250_000)
    throw new Error("Only text files up to 250 KB can be opened.");
...
export async function applyWrite(root, change) {
  ...
  let current = null;
  try {
    current = await readFile(root, change.path);
  } catch (e) {
    if (e.code !== "ENOENT") throw e;
  }
  if ((current === null ? null : hash(current)) !== change.beforeHash)
    throw new Error(
      "The file changed since this proposal. Reject it and ask the agent to read the latest version.",
    );
...
export async function runCommand(
  root,
  command,
  { signal, onData = () => {}, timeout = 60000 } = {},
) {
  if (typeof command !== "string" || !command.trim() || command.length > 8000)
    throw new Error("Enter a command under 8,000 characters.");
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      finish();
      resolve({ code, output, timedOut, cancelled: Boolean(signal?.aborted) });
    });
  });
}
